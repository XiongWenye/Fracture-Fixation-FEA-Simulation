...
def plot_model(nodes, elements, material_props, step, U=None, stresses=None):
    ...
    # 保存图像
    plt.savefig(f"output/model_step_{step:02d}.png")
    plt.close()
    ...
    if stresses is not None:
        ...
        # 保存应力云图
        plt.savefig(f"output/stress_step_{step:02d}.png")
        plt.close()
...
def simulate_healing():
    ...
    for step in range(num_steps):
        ...
        # 保存图像路径
        model_images.append(f"output/model_step_{step:02d}.png")
        stress_images.append(f"output/stress_step_{step:02d}.png")
    ...
    def create_animation(image_files, output_name, fps=2):
        ...
        ani.save(f"output/{output_name}.gif", writer="pillow", fps=fps)
        plt.close()
