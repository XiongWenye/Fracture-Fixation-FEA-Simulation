...
def plot_model(
    ...
    # 静态变量用于收集每2步的图像
    if not hasattr(plot_model, "collected_steps"):
        plot_model.collected_steps = []
        plot_model.collected_imgs = []
    ...
    if stresses is not None:
        ...
        # 只收集步数为2的倍数的图像
        if step % 2 == 0:
            plot_model.collected_steps.append(step)
            plot_model.collected_imgs.append((xi, yi, zi, z.min(), z.max()))
    ...
    if hasattr(plot_model, "collected_steps") and step == 19:
        n = len(plot_model.collected_imgs)
        ncols = 5
        nrows = 2
        ...
        axes = axes.flatten()
        # 统一颜色范围
        vmin = min([img[3] for img in plot_model.collected_imgs])
        vmax = max([img[4] for img in plot_model.collected_imgs])
        for i, (xi, yi, zi, _, _) in enumerate(plot_model.collected_imgs):
            ax = axes[i]
            ...
            ax.set_title(f"Step {plot_model.collected_steps[i]}")
            ax.set_xlabel("Length (m)")
            if i % ncols == 0:
                ax.set_ylabel("Height (m)")
        # 隐藏多余的子图
        ...
        plt.close()
        # 清理静态变量
        del plot_model.collected_steps
        del plot_model.collected_imgs
